...
`;
        ...
        if (isAnthropic) {
          ...
          // OpenRouter uses full model strings like "anthropic/claude-3-opus"
        ...
        console.log(`[generate-ai-code-stream] Using provider: ${isAnthropic ? 'Anthropic' : isOpenRouter ? 'OpenRouter' : isGoogle ? 'Google' : isOpenAI ? 'OpenAI' : 'Groq'}, model: ${actualModel}`);
        console.log(`[generate-ai-code-stream] AI Gateway enabled: ${isUsingAIGateway}`);
        console.log(`[generate-ai-code-stream] OpenRouter enabled: ${isUsingOpenRouter}`);
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            const isOpenRouterError = isOpenRouter && (
              streamError.message?.includes('Insufficient credits') ||
              streamError.message?.includes('quota') ||
              streamError.message?.includes('Rate limit') ||
              streamError.message?.includes('model not found')
            );
            ...
            if (retryCount < maxRetries && isRetryableError) {
              ...
              await sendProgress({
                type: 'info',
                message: isOpenRouterError
                  ? `OpenRouter error: ${streamError.message}. Retrying...`
                  : `Service temporarily unavailable, retrying (attempt ${retryCount + 1}/${maxRetries + 1})...`
              });
              ...
              // If OpenRouter fails with quota, try a different provider
              if (isOpenRouterError && retryCount === maxRetries) {
                console.log('[generate-ai-code-stream] OpenRouter quota/credits issue, trying fallback...');
                await sendProgress({
                  type: 'info',
                  message: 'OpenRouter quota exceeded. Try selecting a different model or check your credits at openrouter.ai'
                });
              }
            } else {
              ...
              await sendProgress({
                type: 'error',
                message: `Failed to initialize ${isOpenRouter ? 'OpenRouter' : isGoogle ? 'Gemini' : isAnthropic ? 'Claude' : isOpenAI ? 'GPT-5' : isKimiGroq ? 'Kimi (Groq)' : 'Groq'} streaming: ${streamError.message}`
              });

              // Provide helpful info based on the provider
              if (isOpenRouter) {
                await sendProgress({
                  type: 'info',
                  message: 'Tip: Check your OPENROUTER_API_KEY and ensure you have credits. Try a different free model.'
                });
              } else if (isGoogle) {
                await sendProgress({
                  type: 'info',
                ...
                await sendProgress({
                  type: 'info',
                  message: 'Tip: Check your ANTHROPIC_API_KEY and ensure you have API credits.'
                });
              }
